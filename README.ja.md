...
| 問い | 必要なもの | 現在地 |
|---|---|---|
| **Q1.** どの酵素反応が環境保全寄与が大きいか | 反応クラスを横断して比較可能な指標とランキング | **指標は完了、被覆は拡大中**。製品1 kgあたりの削減量（kg-CO₂e）で順位付けできる。クラスは20個に増え、6,863/18,558件（37.0%）を照合、3,704件（20.0%）を判定確定。正直な上限は[「被覆率はどこまで伸びるか、なぜそれ以上は伸びないか」](#被覆率はどこまで伸びるかなぜそれ以上は伸びないか)を参照 |
...
1. ~~クラスを増やす——安くなったので~~ **完了、継続中。** テンプレートは
   ...
   **20個目のクラス`udp-acetylhexosaminyltransferase.yaml`**（UDP-GlcNAc
   ...
   より安定な塩化物の方が有利だからである。最終合計：**6,863/18,558件
   を照合（37.0%）、3,704件を判定確定（20.0%）**——本セッションで初めて
   判定確定率20%を超えた。
...
これは80〜100%がすぐに到達可能という意味ではなく、また現在の37.0%照合
