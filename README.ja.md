...
クラスでは**451件中451件の判定が、同一の1物質に半分以上を負っている**、
...
| 問い | 必要なもの | 現在地 |
|---|---|---|
| **Q1.** どの酵素反応が環境保全寄与が大きいか | 反応クラスを横断して比較可能な指標とランキング | **指標は完了、被覆は拡大中**。製品1 kgあたりの削減量（kg-CO₂e）で順位付けできる。クラスは26個に増え、8,041/18,558件（43.3%）を照合、3,906件（21.0%）を判定確定。正直な上限は[「被覆率はどこまで伸びるか、なぜそれ以上は伸びないか」](#被覆率はどこまで伸びるかなぜそれ以上は伸びないか)を参照 |
...
下表の各行は、判定確定した451件すべてを異なる転化率で採点し直した結果
である：
...
451件中わずか25件しかない。残る426件は、酵素がどれだけ優秀であろうと
判定を失う。** 生き残った25件も、判定を維持するには最小85.3%、中央値
93.3%、最悪100%という転化率を要求する。較正に使った反応
（RHEA:12560、β-アルブチン）自身も426件の側にある——その閾値85.87%は、
プラントの90%を下回るからだ。
...
について**の事実を報告する。全反応の順位範囲が1–451で返ってくるからだ。
...
削減量——である。溶媒回収90%のもとで、下限が正になるのは**451件中25件**
だけだった。そしてこの下限順位は、スクリーニングが本来測定したかった
機構を再現している：上位10件はいずれも保護可能官能基を27〜34個持ち、
首位はN-グリカンの **+4.15 kg-CO₂e/kg** である。これは、この指標が閾値と
同じものを測っていることの検算になっている。

### ここから先の道筋

1. ~~クラスを増やす——安くなったので~~ **完了、継続中。** テンプレートは
   `process_model`（当量で宣言した試薬、反応濃度から導く溶媒量）を宣言
   できるようになった。1本の論文の仕込み量を引用する必要がない。
   パラメータは化学に依存しないので、新クラスは試薬だけ用意して工程を
   継承する。**これが網羅性を解いた鍵**である：文献探索3ラウンドで新規の
   論文由来テンプレートは0本であり、「1クラス1論文」は原理的にスケール
   しない。出荷中の`udp-glucosyltransferase.yaml`クラス自体も、
   ガラクトースを追加した時と同じ方法で無料で拡大した：ヘキソース-
   ヌクレオチド供与体をさらに6種（GDP-マンノース、ADP-グルコース、
   GDP-グルコース、UDP-ガラクトフラノース、dTDP-グルコース、
   CDP-グルコース——いずれも同じ162.14 g/molのヘキソース転移を、
   異なるヌクレオチドが運ぶだけ）追加し、新規研究ゼロで72件の照合と
   63件の判定確定を増やして406/388から**478/451**へ拡大した——
   以下のQ1/Q2の発見はすべてこの拡大後のデータで再計算したものであり、
   そのまま流用してはいない。
   同じ方式でさらに6クラスを構築した——論文の引用は一切ない。
   `sam-methyltransferase.yaml`（EC 2.1.1、SAM依存のO/N/S-メチル化）は
   449件を照合、351件を判定確定。`nad-oxidoreductase.yaml`（EC 1.1.1、
   NAD(P)+依存の酸化、**Rhea最大のEC-3群**）は515件を照合したが、
   **判定確定は0件**——正直に報告する。工程モデルが小規模すぎて、
   補酵素の広い（根拠のない）境界に対して判定を確定できないためであり、
   試薬当量を水増しして無理に判定を出すことはしていない。
   `atp-kinase.yaml`（EC 2.7.1、ATP依存のリン酸化）は209件を照合、
   202件を判定確定——すべて酵素側が有利と決定的に判定された。ChEBIが
   リン酸化産物を中性のジ酸ではなく**二価アニオン**として記録している
   電荷状態の慣習を突き止め、質量差を教科書値の79.980から実測値の
   77.963へ補正したのもこのクラスでのことである。`dmapp-prenyltransferase.yaml`
   （EC 2.5.1、DMAPP依存のプレニル化）は47件を照合、38件を判定確定——
   これもすべて決定的判定。同じ補酵素を消費しながら異なる変換である
   鎖伸長反応とDMAPPホモ二量化という、もう2つの本物の交絡を正しく
   除外した（誤って判定確定にはしなかった）。`acetyl-coa-acyltransferase.yaml`
   （EC 2.3.1、アセチルCoA依存のO/N-アセチル化）は138件を照合。今度は
   **アクセプター側**が余分なプロトンを持つという、2件目の電荷状態の
   統一を見つけた（プロダクト側ではなく）。そして2件目の正直な非決定
   結果——解決できた124件はすべて判定不能。NAD(P)+クラスとは別の理由:
   アクセプターが小さいため、アセチルCoAの最も安い想定値でも製品1kg
   あたりの補酵素コストが相対的に大きくなる。`udp-glucuronosyltransferase.yaml`
   （UDP-グルクロン酸依存のグルクロン酸抱合——実在の薬物代謝UGTファミリー）
   は102件を照合、95件を判定確定——すべて決定的判定。プロトン1個分ずれた
   **3件目の電荷状態の分裂**を、他の2件と同じ方法で統一した。さらに
   DMAPPクラスの兄弟を3つ——`gpp-prenyltransferase.yaml`、
   `fpp-prenyltransferase.yaml`、`ggpp-prenyltransferase.yaml`
   （残る3種のアリル二リン酸プレニル供与体、DMAPPの1単位ではなく
   2/3/4イソプレン単位を転移——11/13/9件を照合、10/2/4件を判定確定）を
   構築した。FPPは正直な発見である：判定確定はわずか13件中2件
   （15.4%、DMAPPの80.9%とは逆）——実際の化学の大半が鎖伸長やFPP
   ホモ二量化であり、外部求核剤への転移ではないためだ。水増しせず、
   小さいクラスは小さいクラスとして正直に報告した。この3クラスの構築中に
   全クラス共通のバグも見つかった：アクセプター特定処理は反応式の
   「生成物」側の裸のプロトンは除外していたが「反応物」側は除外して
   おらず、プロトンを真の共反応物として必要とする反応（Rhea全18,558件中
   2,673件、14.4%）を静かに識別失敗させていた。左右対称に修正し、
   既存10クラス全てに対して「マッチを追加することしかできず、既存の
   マッチを失うことはない」ことを確認してから適用した——3クラスがそれぞれ
   1件ずつ判定確定を増やした。そして11個目、`o2-monooxygenase.yaml`
   （EC 1.14.13、芳香族/脂肪族水酸化、Baeyer-Villiger酸化、スルホキシド化、
   N-酸化——198件を照合、134件を判定確定、すべて決定的判定。4件目の
   独立した電荷状態の分裂）は、真に**2つの**補酵素を必要とする最初の
   クラスである：O2が生成物に取り込まれる原子を供給し、NAD(P)Hが同じ
   ステップで機構を駆動する。これまでのアーキテクチャではこれを表現
   できなかったため、`ClassTemplate` に `unpriced_co_cofactor_chebi`
   を追加した——NAD(P)Hはアクセプター探索からは除外されるが、意図的に
   価格をつけない。これは捏造したゼロではなく明示された欠落である
   （このクラスの酵素側はNAD(P)Hの実際の再生コスト分だけ過小評価
   されている）ことを、このクラスが生成するすべてのレポートで警告する。
   12個目、`p450-monooxygenase.yaml`（EC 1.14.14、シトクロムP450および
   関連するヘムチオラートモノオキシゲナーゼ——Rhea単体で最大のO2消費
   EC群、EC 1.14.13より大きい——256件を照合、176件を判定確定、すべて
   決定的判定）は、同じ`unpriced_co_cofactor_chebi`機構を別の電子供与体
   （`CHEBI:57618`——Rheaの反応式が生物学的文脈に応じて「還元型
   [NADPH-ヘムタンパク質レダクターゼ]」「FMNH2」「還元型フラボドキシン」
   と様々に呼ぶ同一のChEBI実体、およびFADH2）で再利用したものである。
   13個目、`o2-desaturase.yaml`（EC 1.14.19、脂肪酸アシル不飽和化——
   115件を照合、92件を判定確定、すべて決定的判定）は、2つのモノ
   オキシゲナーゼクラスとO2補酵素を共有しながら**逆の**質量シグネチャー
   を狙う：このEC群は酸素原子を挿入するのではなく水素を2個除去して
   C=C結合を形成する（−2.016 g/mol、NAD(P)+依存酸化還元クラスが
   狙う2H損失シグネチャーと同一）ため、モノオキシゲナーゼ2クラスの
   mCPBA工程ではなく同クラスの工程モデルを再利用した。14個目、
   `o2-dioxygenase.yaml`（EC 1.13.11、電子供与体を全く伴わずO2の
   両原子を取り込むジオキシゲナーゼ化——102件を照合、59件を判定確定、
   すべて決定的判定）は共補酵素を必要とせず、これまで4回見つかった
   電荷状態パターンの3段階版を示す（32.0／30.99／29.98 g/mol、
   UDP-グルクロン酸転移酵素クラスの2分裂と同じ手法で拡張した許容誤差に
   より統一）。15個目、`2og-dioxygenase.yaml`（EC 1.14.11、Fe(II)/
   2-オキソグルタル酸依存ジオキシゲナーゼ——プロリル/リシル水酸化酵素、
   ジベレリンおよびクラブラン酸生合成——101件を照合、60件を判定確定、
   すべて決定的判定）は、モノオキシゲナーゼクラスと同様に酸素原子を
   1個挿入するが、もう1個の酸素原子は共基質2-オキソグルタル酸を
   酸化的脱炭酸してコハク酸+CO2にする——NAD(P)Hを還元するのではない。
   同じ機構で価格をつけない扱いとした、5件目の異なる共補酵素の実体
   である。16個目、`ferredoxin-monooxygenase.yaml`（EC 1.14.15、
   ステロイド/胆汁酸水酸化酵素、カンファーモノオキシゲナーゼ、
   アルカン水酸化酵素——72件を照合、50件を判定確定、すべて決定的判定）
   は同じmCPBA工程モデルを、3つ目の電子供与体ファミリー（フェレドキシン、
   アドレノドキシン、プチダレドキシン、ルブレドキシン——3つのChEBI IDで
   72件全てを網羅）で再利用したものである。合計：
   **2,815/18,558件を照合（15.2%）、1,724件を判定確定（9.3%）**——1クラス
   時の元々の406/388から拡大した。下の「被覆率の上限」でこの数字が伸びる
   範囲と伸びない理由を述べる。

   **追記（下の訂正済みセクションで上限を再計算した後）：上のすべての
   クラスにおける`ec_prefix`は、必須ではなく不要な追加制約だった。**
   Rhea自身のEC注釈は反応の41.1%しか網羅しておらず、残りの相当数は
   `expected_mass_delta`/`transferred_bond_smarts`だけで構造的に検証
   できる正真正銘のクラス所属である。実装の過程で実在するバグを2つ発見
   し修正した——`_identify`はプロトンと同じ扱いで水を除外していなかった
   ため候補受容体として誤って拾ってしまっていた点（同じ方法で修正し、
   出荷済みの配糖化・グルクロン酸抱合クラスに既に混入していた偽陽性
   3件を除去し、真陽性1件を回収した）と、4つのプレニル基転移酵素クラス
   全てが、イソペンテニル二リン酸1当量の鎖伸長を真正な転移反応と区別
   できずにいた点（質量デルタだけでは真の転移と数学的に見分けが
   つかないため、新しい`excluded_co_cofactor_chebi`フィールドで修正）
   である。両方を修正した上で、10クラス（SAM、ATP-キナーゼ、4つの
   プレニル基転移酵素、O2モノオキシゲナーゼ/デサチュラーゼ/2-オキソ
   グルタル酸/フェレドキシンの各クラス）から`ec_prefix`を除去し、代わり
   に各クラスが実際に必要とする、または排除すべき共補酵素で区別する
   ようにした——`excluded_co_cofactor_chebi`と対をなす新しい
   `required_co_cofactor_chebi`フィールドである。10クラス全てで、以前
   判定確定していた反応が1件も失われないことを検証し、新たに判定確定
   した反応は実際の反応式を1件ずつ確認した。訂正後の合計：6,505/18,558
   件を照合（35.1%）、3,305件を判定確定（17.8%）——クラスごとの内訳は
   `docs/screening.md`、各クラスの検証済み数字は
   `data/reaction-classes/*.yaml`のファイルヘッダを参照。

   **17個目のクラス`paps-sulfotransferase.yaml`**（3'-ホスホアデニリル
   硫酸依存の硫酸化——フェノール、アルコール、アミン、グリコサミノ
   グリカン/ステロイド/胆汁酸の水酸基への硫酸基転移。130件を照合、128件
   を判定確定、すべて決定的判定）も、最初からEC制約なしで同じ方法で
   追加した：質量デルタ検査だけで（正味+79.056 g/mol、水素を硫酸基が
   置換）、PAPSを消費する130件中128件を、混同するケースが1件も見つから
   ないまま分離できる——本プロジェクトが構築したクラスの中で最もS/N比が
   高い。化学対応工程はSO3-ピリジン錯体をピリジン溶媒中で用いる、教科書
   的な穏和な硫酸化剤である。更新後の合計：6,635/18,558件を照合
   （35.8%）、3,433件を判定確定（18.5%）。

   **18個目のクラス`coa-ligase.yaml`**（ATP/GTP依存のCoAチオエステル形成
   ——酸-チオールリガーゼ機構。156件を照合、**判定確定0件**）は、
   NAD(P)+酸化還元酵素クラスやアセチルCoAアシル基転移酵素クラス自身が
   既に記録している正直な非結果の、さらに極端な版である：CoAの転移質量
   （746.502 g/mol、アシルCoAチオエステル全体を形成）は本プロジェクトが
   構築したクラスの中で最大であり、価格未評価のあらゆる補酵素が使う同じ
   広い（根拠のない）[0.5, 100]の補酵素上限に当てはめると、酵素側だけ
   の不確実性幅（機能単位あたり[0.37, 74.6] kgCO2e）が、化学ルート側で
   ありうるどんな足跡とも重なり合うほど広い——僅差ではなく、この上限
   では構造的に判定不能である。156件すべてが質量デルタで正しく分離
   され（除外0件）、判定確定がゼロでも本物の検証済みQ1被覆ではある
   ものの、ユニークな照合被覆への限界的寄与はやはり小さい（+156では
   なく+4）。理由は、`ec_prefix`を除去した後の`atp-kinase`自身の拡大
   済み照合集合に、この156件の大半が既に含まれているためである（両方
   ともATPを要求するが、`atp-kinase`自身の質量デルタ検査がCoAチオエス
   テルの約746 g/molという追加分をその77.963という目標値から大きく
   外れると正しく棄却するため、二重に判定確定することはない）。最終
   合計：6,639/18,558件を照合（35.8%）、3,433件を判定確定
   （18.5%）——17クラス時点の数字とこの精度では変わらない。オーバー
   ラップと判定不能の両方を数えに入れた、このクラスが実際に追加する
   限界的被覆の小ささを正直に反映した数字である。

   **系統的な回収であり、新規クラスではない：AH2。** RheaはCHEBI:17499
   （「AH2」）を「特定できない還元型供与体」を表す独自の汎用プレース
   ホルダーとして使っている——キュレーターがモノオキシゲナーゼ型反応に
   還元剤が必要だと分かっていても、その具体的な正体が分からなかった
   箇所である。本プロジェクトが構築したあらゆるクラスにまたがって169件
   の反応が、AH2が受容体探索から除外されずに残っていたせいで単一受容体
   への解決をブロックされていた——水やプロトンの除外と同じ形だが、今回
   は普遍的にではなくクラスごとに判断が必要だった。なぜならAH2は、
   クラスによって本当に異なる変換を表すからである（`sam-methyltransferase`
   ではラジカルSAM化学、`atp-kinase`ではtRNA硫黄リレーやコバラミン
   生合成——どちらも正しく除外したままにする）。検証の結果、本当に
   意味があったのは2クラスだけだった：`o2-monooxygenase`（判定確定+64、
   失われたもの0件、他のO2クラスとの二重判定確定も0件）と
   `2og-dioxygenase`（+1）。`o2-dioxygenase`自身の除外リストにもAH2を
   追加した——この64件が本来別のクラスに属することが分かった以上、
   「照合」を正直な数字に保つためである。着地前に1つバグを発見して
   修正した：AH2を両クラスの`required_co_cofactor_chebi`に独立に
   追加すると、`required_co_cofactor_chebi`はOR論理であるため、
   `o2-monooxygenase`と`2og-dioxygenase`が同じ64件を一時的に二重判定
   確定してしまった——修正は、AH2が本当に代替供与体であるクラス
   （`o2-monooxygenase`）でのみ`required`に加え、既に別の補酵素が実際の
   ゲート役を果たしているクラス（`2og-dioxygenase`、引き続き
   2-オキソグルタル酸単独でゲートする）では価格未評価のみに留める、
   というものである。この修正後の合計：6,639/18,558件を照合
   （35.8%）、3,498件を判定確定（18.8%）。

   **19個目のクラス`gdp-fucosyltransferase.yaml`**（GDP-フコース依存の
   フコシル化——77件を照合、76件を判定確定、すべて決定的判定）は、
   まだ被覆していないあらゆる補酵素を質量デルタの均質性で直接調査する
   スクリプト（各候補の実際の受容体/生成物デルタを調べるもので、
   EC群の調査ではない）で見つけた——1つずつ手探りで選ぶのではなく。
   本セッションで最もクリーンな新規クラスである：77件中76件が
   146.14 g/mol（フコシル基）から1プロトン以内に収まり、残る1件は
   GDP-フコース・シンターゼ（GDP-フコースを「作る」生合成経路であり、
   質量デルタで正しく別の変換として除外される）だった。RHEA:14257
   （本プロジェクト自身のQ3対象リストが既に名指ししているヒトミルク
   オリゴ糖のフコシル化）を含む。化学対応工程は、実在し商業カタログに
   載っているグリコシルブロミドドナー（CAS 16741-27-8、Web検索で確認
   済み）を用いたケーニヒス-クノール型フコシル化で、
   `udp-glucuronosyltransferase`自身のドナーと同じ機構である。同じ調査
   でさらにクリーンなシアリル基転移酵素の候補も見つかった（CMP-シアル酸、
   122件、純度97%超）——まだ構築していない。シアリル化ドナーは各研究室
   でカスタム合成されることが多く、フコシルブロミドのように単純で検証
   可能な商業CAS番号があるわけではないため、推測で埋めるのではなく、
   文書化された候補として残してある。このクラスまでの合計：6,715/18,558件
   を照合（36.2%）、3,574件を判定確定（19.3%）。

   **20個目のクラス`udp-acetylhexosaminyltransferase.yaml`**（UDP-GlcNAc
   /UDP-GalNAc依存の配糖化——152件を照合、130件を判定確定、すべて決定的
   判定）は、`udp-glucosyltransferase`がUDP-グルコース/UDP-ガラクトース
   を統合したのと同じ方法で2つのドナーを統合する：C4エピマーであり、
   転移する正味の質量が同一（203.194 g/mol）だからである。今回の調査
   で見つかった上位候補の中では最もクリーンでない——旗艦の配糖化クラス
   群が達成する95%超ではなく85.5%と、切り上げずに実際の数字を報告して
   ある。この補酵素ペアには単純な質量デルタ検査で分離しなければならない
   本物の競合化学が存在するからである：ドリコリルリン酸へのGlcNAc-1-
   リン酸転移（RHEA:13289、EC 2.7.8.15、GlcNAcだけでなくホスホ糖全体を
   転移する別の酵素ファミリーで、質量が異なる）や、ドナー自身のウロン
   酸への酸化（RHEA:13325、NAD+依存）——どちらも正しく除外されており、
   まとめて含めてはいない。化学対応工程は他の糖ドナー系クラスが使う
   臭化物ではなくグリコシル塩化物ドナー（CAS 3068-34-6、Web検索で確認
   済み）を用いる。この糖に特有のC2アセトアミド基の隣接基関与により、
   より安定な塩化物の方が有利だからである。このクラスまでの合計：
   6,863/18,558件を照合（37.0%）、3,704件を判定確定（20.0%）——本
   セッションで初めて判定確定率20%を超えた。

   **`gdp-fucosyltransferase.yaml`を拡張、新規クラスではない：UDP-
   ラムノース。** フコースとラムノースはどちらもL配置の6-デオキシ
   ヘキソースであり——立体化学は異なるが式は同一、転移する質量も同一
   （146.14 g/mol、RHEA:61160「ケルセチン→クエルシトリン」で検証済み）
   ——そこでUDP-beta-L-ラムノースを21個目のクラスとして構築するのでは
   なく、フコシル化クラス自身の`cofactor_chebi`リストに追加した。同じ
   統合手法は既に3回使われている（UDP-グルコース/UDP-ガラクトース、
   UDP-GlcNAc/UDP-GalNAc、そして今回のGDP-フコース/UDP-ラムノース）。
   ラムノースを消費する15件中14件が同一のデルタに乗る（フラボノイドや
   サポニンのラムノシル化——ミリセチン、ケルセチン、ケンフェロールなど）
   ——残る1件は成長中のラムノガラクツロナンの鎖伸長であり、正しく除外
   される。統合を反映して「6-デオキシヘキソース-ヌクレオチド依存の配糖化」
   に改名した。この統合までの合計：6,878/18,558件を照合（37.1%）、
   3,718件を判定確定（20.0%）。

   **21個目のクラス`udp-xylosyltransferase.yaml`**（UDP-キシロース依存の
   キシロシル化——24件を照合、22件を判定確定、すべて決定的判定）は、
   質量デルタ調査のクリーンな候補群から見つかった3番目にして最後の
   クラスである。キシロースはヘキソースではなくペントースであるため、
   同じケーニヒス-クノール機構を持ちながら上のいずれの糖クラスとも
   統合できない——転移質量132.12 g/mol、RHEA:22244（ケンフェロール→
   ケンフェロール3-O-beta-D-キシロシド）で検証済み、ヘキソースドナー
   より炭素1個分軽い。EGF様/Notchドメインへのタンパク質O-キシロシル化
   や、フラボノイド・サポニンのキシロシル化を含む。除外された2件：
   別の正味質量を持つホスホキシロシル転移1件と、成長中のプロテオグリカン
   結合領域の鎖伸長1件。このクラスまでの合計：6,902/18,558件を照合
   （37.2%）、3,740件を判定確定（20.2%）。

   **22個目のクラス`cmp-sialyltransferase.yaml`**（CMP-シアル酸依存の
   シアリル化——122件を照合、119件を判定確定、すべて決定的判定）は、
   質量デルタ調査で見つかった未構築候補の中で最大のものであり、単純で
   検証可能な商業シアリル化ドナーを見つけるのに他の糖ヌクレオチド系
   クラス（アセチル化ハロゲン化物ドナー）より時間がかかったために
   後回しにしていた。転移する正味質量は290.25 g/mol（シアリル基）で、
   RHEA:11836（673.598 − 383.350 = 290.248 ちょうど）で検証済み、
   ガングリオシド受容体（RHEA:18021、GM1 → GD1a）や糖脂質受容体
   （RHEA:18417）でも同一であることを確認した。本物の二重転移
   クラスターも存在する——分岐N-グリカン受容体に一度に2つのシアリル
   基を導入する4件の反応が、ちょうど2×290.25=580.50に乗る——これは
   SAMクラスのジ/トリメチル化と同じ`cofactor_coeff`機構で正しく
   判定確定される。化学対応工程は他の糖ドナー系クラスのケーニヒス-
   クノール型化学よりも本質的に難しい（ネイバリング基関与のない
   ケトース性アノマー中心）ため、ハロゲン化物ではなくNIS/TfOHで
   活性化するチオグリコシドドナー（CAS 155155-64-9）を用いており、
   その現実の難しさを反映してステージ収率も低く設定してある（他の
   糖ドナー系クラスの80%ではなく65%）。除外された3件：2件
   （RHEA:79555、RHEA:81827）は同じ補酵素が関与する別の変換である
   アセチルCoAによるシアル酸自身の側鎖のO-アセチル化であり、1件
   （RHEA:16145）はO2/シトクロムb5によるN-グリコリル体への水酸化で、
   この単純な二反応物モデルが扱わない三反応物の形である。このクラスまでの
   合計：7,024/18,558件を照合（37.8%）、3,859件を判定確定（20.8%）。

   **23個目のクラス`cdp-cholinetransferase.yaml`**（CDP-コリン依存の
   ホスホコリン転移——18件を照合、17件を判定確定、すべて決定的判定）は、
   同じ調査でシアル酸クラスと一緒に見つかったより小さい方の候補で、
   本セッションでは先にシアル酸側を優先し後回しにしていた。転移する
   正味質量は165.13 g/mol（ホスホコリン基）で、RHEA:32939
   （ジアシルグリセロール→ホスファチジルコリン）で検証済み、セラミド
   受容体（RHEA:16273、→スフィンゴミエリン）やタンパク質セリン受容体
   （RHEA:56080、Legionellaエフェクター AnkXによるRab1のホスホコリン化）
   でも同一であることを確認した——それぞれ異なるEC番号でありながら
   同一の質量デルタ機構で統一されており、そのためec_prefixは宣言して
   いない。化学対応工程はAneja法——合成ホスファチジルコリンの確立された
   合成法であり、環状クロロリン酸エステル（2-chloro-2-oxo-1,3,2-
   dioxaphospholane、CAS 6609-64-9）でリン酸化した後、トリメチルアミン
   （CAS 75-50-3）で開環する。どちらもWeb検索で確認済み。除外された1件
   （RHEA:32487）はCDP-コリン自身の単純な加水分解であり、補酵素とH+と
   H2Oを除外すると有機受容体が残らない。このクラスまでの合計：
   7,042/18,558件を照合（37.9%）、3,876件を判定確定（20.9%）。

   **24個目のクラス`nadph-ketoreductase.yaml`**（NADPH依存のカルボニル
   還元——687件を照合、0件を判定確定、もう一つの「正直な非決定」）は、
   `nad-oxidoreductase`の鏡像である：酸化型ではなく還元型の補酵素
   （NADPH）にマッチさせることで逆方向（ケトン/アルデヒド→アルコール）
   を捉える。NADPHの主要な質量デルタクラスター（+2.016 g/mol）は、
   実は質量が同じ2つの異なる還元——カルボニル還元とアルケン還元——の
   混合であることが判明した。これはSAMクラスの炭素/ヘテロ原子メチル化
   と同じ混同であり、同様にボンドチェックで解決した：
   `transferred_bond_smarts: "[CX4][OX2H]"`（新しいC-OH）を宣言し、
   実際のRhea構造に対して直接検証した。687件中148件（21.5%）が構造的
   に綺麗なカルボニル還元であり、これは本物の被覆であるが、**そのすべて
   が判定確定ではなく判定不能**である：NADPHはNAD(P)+と同程度に重く
   （約745 g/mol）、その広く根拠のない補酵素バウンドが、
   `nad-oxidoreductase`、`acetyl-coa-acyltransferase`、`coa-ligase`で
   既に記録されているのと同じ「正直な非決定」を生む。化学対応工程は
   水素化ホウ素ナトリウム（CAS 16940-66-2）をメタノール中で用いる——
   カルボニル還元の標準的な水素化物源であり、ボンドチェックが除外する
   アルケン還元をカバーすると主張するものではない。本セッションの
   合計：7,729/18,558件を照合（41.6%）、3,876件を判定確定（20.9%）。

   **25個目のクラス`nadh-ketoreductase.yaml`**は、上記NADPHクラスの
   直接の兄弟である——同じセッション、同じボンドチェックの理屈、同じ
   水素化ホウ素ナトリウムの化学対応工程で、NADPHの代わりにNADHに
   マッチさせる。278件を照合（NADPHの687件より小さい）、構造的に
   綺麗なカルボニル還元は18件、**その18件すべてが判定不能**——同じ
   「正直な非決定」、同じ重い補酵素という理由（NADHは約665 g/mol）。
   このクラスまでの合計：8,007/18,558件を照合（43.1%）、3,876件を判定
   確定（20.9%）。

   **26個目のクラス`hcn-cyanohydrin.yaml`**（HCN依存のシアノヒドリン
   形成——34件を照合、30件を判定確定、すべて決定的判定）は、より広い
   2回目の調査で見つかった、珍しく小さく珍しく綺麗な候補である。
   ヒドロキシニトリルリアーゼの化学：HCNがアルデヒドまたはケトンに
   付加してシアノヒドリンを形成し、正味質量は27.026 g/mol（HCN自身）、
   RHEA:77427（ベンズアルデヒド→(S)-マンデロニトリル）で検証済み。
   ec_prefixは宣言していない——本物のメンバーはすべて今回のRheaで
   EC番号が一切付与されていない。化学対応工程はシアン化カリウム
   （CAS 151-50-8）を酢酸で緩衝した標準的な教科書的合成——純粋な
   HCNガスではなく、ベンチで安定な代替物を使う。本セッションの
   最終合計：**8,041/18,558件を照合（43.3%）、3,906件を判定確定
   （21.0%）**——セッション開始時の2,815/1,724件（15.2%/9.3%）から
   拡大した。
2. **脱溶媒に本気の化学テンプレートを作る**（フェアな勝負）。両ルートに
   ...
   **固定化の項は未申告**——酵素の仕込み量・再利用回数・酵素生産の
   ...
   補正前は効率99%でも酵素が451件全勝していたが、実際の仕込み量では
   99%で**全451件が判定不能**になる。あの全勝は酵素への過少課金の産物
   だった。一方、論文が報告する実数値同士——UDP-グルコース240回転と
   工業標準の溶媒回収90%——で採点すると、**451件すべてが保証された削減
   量を持つ**。
   ...
   **溶媒の「回収」と「回避」は別のレバーである。** 脱溶媒に本気の酵素法
   ...
   出典から作ったテンプレートが要る。
3. **出典が得られる場合は論文由来クラスも作る**（任意、`process_model`
   による被覆より優先度は下）。各クラスに実在の公表手順が1本ずつ必要で、
   全クラスで同一の規則で選ばないと、クラス横断の順位は化学ではなく
   「各論文の著者がどれだけ本気だったか」を測ることになる。
4. **商業プロセスを位置づける**（Q3）。ヒトミルクオリゴ糖のフコシル化
   （RHEA:14257 ほか）、アントシアニン配糖化（RHEA:20093）、β-アルブチン
   （RHEA:12560）など、既に商業生産されている酵素反応をRhea IDに対応
   付け、上記の保証下限順位における分位点を報告する。

### 被覆率はどこまで伸びるか、なぜそれ以上は伸びないか

正直に述べる。上の数字を見れば当然出る問いだからだ：**クラステンプレート
方式によるRheaのスクリーニングは、網羅的と呼べる被覆率には原理的に
到達できない。これは労力の問題ではなく、データについての事実である。**

**訂正：以下の段落はもともと上限を43.5%、目指すべき正直な数字を30〜40%と
主張していたが、どちらも誤りだった**——都合で選んだ16種の補酵素だけを
数えた結果生じた誤差であり、真の構造的到達可能性を測ったものではな
かった。正しくは、Rheaのあらゆる反応の左辺に現れる種を、共起する反応数
で重み付けしてすべて数え、真に遍在するスペクテータ（H₂O、H⁺、および
ごく一部の普遍的金属イオン）だけを除外する。この方法で測ると、**Rhea
の11.4%（2,110件）は左辺の構成種を他のどの反応とも一切共有していない**
——これがクラステンプレート方式では原理的に到達不能な唯一の部分であり、
どれだけクラスを作り込んでも変わらない。真の上限は**43.5%ではなく
88.6%である。**

元の段落のもう一つの誤りは、O₂を「単一クラスとしてテンプレート化
できない」としていた点である。それはO₂**単体**については正しいが、実際
のO₂消費反応でO₂だけが使われることはなく、必ず特定の電子供与体
（NAD(P)H、還元型フラビン、還元型鉄硫黄クラスター、2-オキソグルタル酸、
あるいは供与体なし）を伴う——そしてその共補酵素の正体こそが、明確で
機構的な判別要因になる。本プロジェクトは実際にこの原理に基づく6クラス
（`o2-monooxygenase`、`p450-monooxygenase`、`o2-desaturase`、
`o2-dioxygenase`、`2og-dioxygenase`、`ferredoxin-monooxygenase`）を
構築済みで、O₂消費反応2,931件をカバーし、`required_co_cofactor_chebi`／
`excluded_co_cofactor_chebi`（2つ上のセクションの「追記」を参照）に
よってEC接頭辞に頼らず機構同士を正しく分離している。

これは80〜100%がすぐに到達可能という意味ではなく、また現在の43.3%照合
／21.0%判定確定と88.6%という構造的上限とのギャップが小さい、あるいは
容易だという意味でもない——それは大規模かつ複数年にわたる取り組みで
あり（補酵素が「テンプレート化する価値がある」水準、すなわち5件以上の
反応で共起する層に到達するには、あと約800個のクラステンプレートが必要
と見積もられる）。しかし、データそのものによって30〜40%に頭打ちされて
いるわけではなく、その上限を固定的なものとして報告すること自体が、本
プロジェクトの基準に照らせば捏造された数字だった——ここで沈黙のまま
放置せず訂正する。元の主張のうち今も正しい部分：*今すぐ*80%の被覆率を
報告するために「補酵素＋構造検査」という方法論を緩めることは、まさに
本プロジェクトが拒否するために存在するたぐいの近道である。重要な数字は
今も**照合ではなく判定確定**であり、正直に検証された21.0%は捏造された
80%より価値がある。88.6%へのギャップを埋めるのは、基準を下げることで
はなく、同じ厳密さでクラスを作り続けることである。
...
コース、その立体異性体UDP-ガラクトース、または6種の兄弟ヘキソース-
ヌクレオチド供与体のいずれかを消費する糖転移反応478件すべてを、実在
する公表済みの化学合成手順1つに対してスクリーニングした。かかった
時間は約1秒：
...
した数字である。たとえば閾値が84.56%の反応なら、化学プラントが溶媒の
84.56%未満しか回収していない限り酵素法の方が低炭素だと確定的に言え、
84.56%を超えて回収するようになると、もう酵素法が有利だとは言い切れなく
なる。

これを見出しの数値にしたのは、「勝った反応の数」を見出しにすると誤解を
招くからだ。実際の工業的な蒸留による溶媒回収率は、普通90〜95%に達する。
つまり、478件の反応の閾値（84.56%〜91.54%）は、軒並みこの実プラント
水準を下回っている。言い換えると、**判定が確定した451件のうち、実際の
工場が普通に達成できる99%という回収率に耐えられるものは1件もなかった。**
だから正直な結論は「酵素が451回勝った」ではない。**「この反応クラスに
...
いるクラステンプレート1つに対して478件を採点する時間であり、純粋な
...
1つ追加するのは無料にはならない——ただし例外もある。元々の406件のうち
143件を占めるUDP-ガラクトースは、新規の調査ゼロでこの同じクラスに
追加できた。UDP-グルコースの立体異性体（分子式も、付加する質量も同一）
だからで、これはRDKitで実際に検証済みであり、名前から類推しただけでは
ない——同じ手口で後からさらに72件（マンノース55件はGDPが運び、残りは
同じグルコースをADP・GDP・dTDP・CDPが運ぶ）を追加できた。どの
