...
| 問い | 必要なもの | 現在地 |
|---|---|---|
| **Q1.** どの酵素反応が環境保全寄与が大きいか | 反応クラスを横断して比較可能な指標とランキング | **指標は完了、被覆は拡大中**。製品1 kgあたりの削減量（kg-CO₂e）で順位付けできる。クラスは16個に増え、2,815/18,558件（15.2%）を照合、1,724件（9.3%）を判定確定。正直な上限は[「被覆率はどこまで伸びるか、なぜそれ以上は伸びないか」](#被覆率はどこまで伸びるかなぜそれ以上は伸びないか)を参照 |
...
マッチングには、反応が認識可能な補酵素を反応物として消費している必要が
ある。Rheaで最も一般的な16種の補酵素——CoA、アセチルCoA、SAM、4状態
すべてのNAD(P)(H)、ATP、ADP、UDP糖類、FAD、FMN、O₂、H₂O₂——のいずれか
を消費する反応を、それ以上の絞り込みなしに合算しても、**18,558件中
8,073件、43.5%にしかならない。** これがこの手法の上限であり、それも
クラス純度のための絞り込み（EC制約、質量デルタ検査、結合検査——出荷済み
の2クラスでそれぞれ449件中98件、478件中27件を除外している）を適用する
前の数字である。O₂だけでこの上限の15.8%を占めるが、O₂は単一クラスとして
テンプレート化できない——酸化、酸素化、ラジカル化学に消費され、共通の
「化学的対応物」が存在しないためで、これは上の
[「語彙が有限だということは同じ意味ではない」](#なぜ18558件が解ける規模の問題になるのか)
で述べたCoAやSAMの不均質性問題を、補酵素レベルでさらに一段悪化させた
ものである。

**Rhea被覆率80%は、この手法を頑張って伸ばせば届く目標ではない——目指す
べき正直な数字は30〜40%に近い。** テンプレート化するに値する規模を持ち、
かつ質量デルタ検査と結合検査を通過するほど均質な、EC-3群から積み上げた
数字である。それでもデータベースの大きな、有用な、そして——生の被覆率と
違って——**比較可能な**部分になる。どの反応クラスに本当の調査を投じる
べきかを判断するのに十分な数字であり、それがそもそもの目的だった
（[「次に作るべきクラスを、頻度ではなくEC番号で選ぶ」](#次に作るべきクラスを頻度ではなくec番号で選ぶ)
の結びを参照）。80%と報告することは、本プロジェクトが実証しようとして
いる「補酵素＋構造検査」という方法論そのものを放棄し、より緩い基準で
マッチさせることを意味する——それはまさに生の補酵素頻度への警告が示す
とおり、化学を黙って混ぜることになる。その取引は、本リポジトリの他の
すべての数字と同じ基準で拒否する：**正直に境界づけられた35%は、捏造
された80%より価値がある。**
