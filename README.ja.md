...
| 問い | 必要なもの | 現在地 |
|---|---|---|
| **Q1.** どの酵素反応が環境保全寄与が大きいか | 反応クラスを横断して比較可能な指標とランキング | **指標は完了、被覆は拡大中**。製品1 kgあたりの削減量（kg-CO₂e）で順位付けできる。クラスは21個に増え、6,902/18,558件（37.2%）を照合、3,740件（20.2%）を判定確定。正直な上限は[「被覆率はどこまで伸びるか、なぜそれ以上は伸びないか」](#被覆率はどこまで伸びるかなぜそれ以上は伸びないか)を参照 |
...
1. ~~クラスを増やす——安くなったので~~ **完了、継続中。** テンプレートは
   ...
   **21個目のクラス`udp-xylosyltransferase.yaml`**（UDP-キシロース依存の
   ...
   結合領域の鎖伸長1件。本セッションの最終合計：**6,902/18,558件を照合
   （37.2%）、3,740件を判定確定（20.2%）**——セッション開始時の
   2,815/1,724件（15.2%/9.3%）から拡大した。
...
これは80〜100%がすぐに到達可能という意味ではなく、また現在の37.2%照合
／20.2%判定確定と88.6%という構造的上限とのギャップが小さい、あるいは
...
今も**照合ではなく判定確定**であり、正直に検証された20.2%は捏造された
