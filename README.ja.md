...
1. **クラスを増やす——安くなったので**（Q1の網羅性）。テンプレートは
   `process_model`（当量で宣言した試薬、反応濃度から導く溶媒量）を宣言
   できるようになった。1本の論文の仕込み量を引用する必要がない。
   パラメータは化学に依存しないので、新クラスは試薬だけ用意して工程を
   継承する。**これが網羅性を解く鍵**である：文献探索3ラウンドで新規の
   論文由来テンプレートは0本であり、「1クラス1論文」は原理的にスケール
   しない。同じ388反応を両モデルで採点すると、工程モデルは酵素の優位性を
   約6分の1にし、**それでも判定は全件生き残る**（388件すべてが保証削減を
   持つ）。EC-3の上位群は 1.1.1（526件）、2.1.1（500件）、2.3.1（382件）。
2. **脱溶媒に本気の化学テンプレートを作る**（フェアな勝負）。両ルートに
   ...
   **溶媒の「回収」と「回避」は別のレバーである。** 脱溶媒に本気の酵素法
   ...
   出典から作ったテンプレートが要る。
3. **出典が得られる場合は論文由来クラスも作る**（任意）。EC-3上位群——
