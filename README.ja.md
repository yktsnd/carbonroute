...
| 問い | 必要なもの | 現在地 |
|---|---|---|
| **Q1.** どの酵素反応が環境保全寄与が大きいか | 反応クラスを横断して比較可能な指標とランキング | **指標は完了、被覆は拡大中**。製品1 kgあたりの削減量（kg-CO₂e）で順位付けできる。クラスは4つに増え、1,579/18,558件（8.5%）を照合、941件（5.1%）を判定確定。正直な上限は[「被覆率はどこまで伸びるか、なぜそれ以上は伸びないか」](#被覆率はどこまで伸びるかなぜそれ以上は伸びないか)を参照 |
...
1. ~~クラスを増やす——安くなったので~~ **完了、継続中。** テンプレートは
   ...
   **判定確定は0件**——正直に報告する。工程モデルが小規模すぎて、
   ...
   77.963へ補正したのもこのクラスでのことである。合計：
   **1,579/18,558件を照合（8.5%）、941件を判定確定（5.1%）**——1クラス
