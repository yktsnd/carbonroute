...
ここまでは全て、ツールが回答を拒む話だった。ここからは**答えを出す**話である
——しかも3件中もっともカバレッジが低い事例で。

2つのルートの順位付けは、どちらかを実測するより簡単な問いであり、データが
悪くてもその関係は変わらない。欠けている係数が**何であるか**を知る必要は
必ずしもない。答えがその値に**依存しなくなる**程度に、その値が**どこにあり
得ないか**が分かればよい。

イブプロフェンの比較（[Grimaldi et al., *ACS Sustainable Chem. Eng.*
**2021**](https://doi.org/10.1021/acssuschemeng.1c02309)）は差分質量の52.9%
しか解決できず、未解決物質は9つある。その各々に「真の係数はこの区間にある」
という**値ではなく境界**を与え、その中のどこを取っても順位が同じかを問う：
...
未解決9物質のうち**7つは、どんな値を取っても答えを変えられない**。比較全体が
イオン液体1つについての1本の不等式に収束する。そしてその最も近い類縁体に
ついて公表されている2つの推定値——**互いに8倍食い違っている** 3.5 と 27.3
——は、この基準を2.0倍と15.9倍のマージンでクリアする。両者は値については
一致しない。順位については全会一致する。

これが要点である：**「境界として使えるだけの質」の要求水準は「計算に使える
だけの質」よりはるかに低く**、比較においてはその低い水準で足りることが多い。
カバレッジは52.9%のまま報告される。境界値が係数になることも、モンテカルロに
入ることも、合計値に触れることも一切ない。

さらにツールは同じ問いを自分の入力自体に向け、真の争点が化学物質ですら
なかったことを明らかにした：
...
論文自身のシナリオは**50%と100%**であり、自らのルートの優位性を「酵素の
リサイクルが高水準であることを条件として」と述べている。2物質分の公開データ、
1物質分の境界区間、そして論文の物質収支だけを使って、`carbonroute` は
ecoinvent と GaBi をフル活用した論文と同じ場所に転換点を独立に導き出した。
...
にある。確定的な答えが得られたことで発覚した ledger 自体の欠陥の記録も
そこに含まれる。
