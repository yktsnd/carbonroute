...
| 問い | 必要なもの | 現在地 |
|---|---|---|
| **Q1.** どの酵素反応が環境保全寄与が大きいか | 反応クラスを横断して比較可能な指標とランキング | **指標は完了、被覆は拡大中**。製品1 kgあたりの削減量（kg-CO₂e）で順位付けできる。クラスは18個に増え、6,639/18,558件（35.8%）を照合、3,498件（18.8%）を判定確定。正直な上限は[「被覆率はどこまで伸びるか、なぜそれ以上は伸びないか」](#被覆率はどこまで伸びるかなぜそれ以上は伸びないか)を参照 |
...
1. ~~クラスを増やす——安くなったので~~ **完了、継続中。** テンプレートは
   ...
   **系統的な回収であり、新規クラスではない：AH2。** RheaはCHEBI:17499
   ...
   というものである。最終合計：**6,639/18,558件を照合（35.8%）、
   3,498件を判定確定（18.8%）**。
...
これは80〜100%がすぐに到達可能という意味ではなく、また現在の35.8%照合
／18.8%判定確定と88.6%という構造的上限とのギャップが小さい、あるいは
...
今も**照合ではなく判定確定**であり、正直に検証された18.8%は捏造された
