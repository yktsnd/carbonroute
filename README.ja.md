...
## データベース全体のスクリーニング：18,500反応を一度に

ここまでは全て、誰かが公表した2つのルートの比較だった。`carbonroute
screen` は、反応データベース全体に対してより狭い問いを立てる：
**各酵素反応について、化学プラントが溶媒を何%回収すれば酵素の優位が
消えるか？**

### なぜ計算量が現実的になるのか

近道ではなく、`compare` を成立させているのと同じ相殺である。2つのルートが
同じ生成物を作るとき、両者に共通するものは全て差分から消える。そして
酵素 vs 化学において消えるのは、まさに調べるのが最も高コストな部分——
反応ごとに異なる基質と生成物——である。残るものは小さく、反復的だ：

| 側 | 差分に残るもの |
|---|---|
| 酵素側 | **補酵素**——UDP-グルコース, NADPH, SAM, アセチルCoA |
| 化学側 | **保護基・活性化剤・塩基・溶媒** |

これはデータについての主張なので、断定せず実測した。
[Rhea](https://www.rhea-db.org/) の18,558反応には14,251種の参加物質が
現れるが、**100反応以上に出現するのはわずか63種、1,000反応以上は12種**
であり、上位30種でデータベース全体の参加スロットの47.8%を占める。その
中身は生化学者が暗記している補酵素リストそのものである。上位30種が
取りこぼす長い裾は、まさに反応ごとの基質と生成物——相殺される側だ。

したがって排出係数を揃える作業量は、その語彙の大きさ——数十物質——で
頭打ちになり、反応数には比例しない。以降は1反応あたり算術1回で済む。
263反応のクラスなら約1秒である。

### 何を報告し、なぜ「判定」を報告しないのか

スクリーニングは、各酵素反応を**クラステンプレート**——実在の公表化学
手順1件を、そのクラスの全基質に適用したもの——と対にする。この外挿こそが
本手法の中心的な仮定であり、だから出力は「本物の `compare` に時間を
かける価値のある反応の順位付きショートリスト」であって、個々の反応に
ついての判定では決してない。

主要な数値を「酵素が何回勝ったか」にしないのも意図的である。クラス
テンプレートは公表された**ベンチスケール**手順に由来し、ベンチ手順は
溶媒を捨てる——同梱テンプレートは生成物1kgあたり800kg超の溶媒を計上して
いる。実プラントはそうしない。そこで報告するのは、各判定が成り立た
なくなる**溶媒回収率の閾値**である。この数値には外部の物差しがある：
工業的な蒸留回収率は90-95%だ。

### UDP-グルコース転移酵素263反応の結果

Cepanec & Litvić (*ARKIVOC* 2008) のHelferich/BF₃グリコシル化を化学側
テンプレートとしてスクリーニングした結果：
...
**判定確定した256反応のうち、99%回収に耐えるものは一つもない。** 分布
全体が実プラントの達成水準以下に収まっている。正直な読み方は「酵素が
256回勝った」ではなく、**「このクラスにおいて、このベンチ手順から
モデル化する限り、酵素の優位性は実在するが限定的であり、工業的な溶媒
リサイクルには耐えない」**である。反証可能であり、勝敗数より有用だ。

このスクリーニングが測定しようとした機構は、分布のばらつきに現れて
いる。閾値は**化学ルートが保護すべき基質側の官能基数とともに上昇する**
——1個以下で85.58%、33個のオリゴ糖で91.43%——化学が保護・脱保護しな
ければならない部位が多いほど、酵素の位置選択性の価値が上がるからだ。
これが構造情報だけから定量化された酵素の優位性である。

### 較正

RHEA:12560——ヒドロキノン + UDP-α-D-グルコース → β-アルブチン——は、
このクラスの一員であると同時に、
[`examples/case-studies/beta-arbutin-chemical-vs-enzymatic/`](examples/case-studies/beta-arbutin-chemical-vs-enzymatic/)
で全出典を追って手作業構築した事例そのものである。スクリーニングはその
生成物質量・受容体・補酵素装入量・判定方向を再現し、テストがそれを固定
している。これがなければ、残り262行は自分自身のテンプレートを報告して
いるだけになる。

手法の詳細は [`docs/screening.md`](docs/screening.md)、実測した補酵素
語彙は [`data/rhea/README.md`](data/rhea/README.md) にある。
...
| `carbonroute screen --template CLASS.yaml --bounds B.yaml` | 反応データベース全体を1つの化学ルートテンプレートに対してスクリーニングし、反応ごとの溶媒回収率閾値を報告する。[`docs/screening.md`](docs/screening.md) 参照。 |
...
- [`docs/bounds.md`](docs/bounds.md) — 係数そのものが欠けているとき、
  境界だけから順位を確定させる方法。
- [`docs/screening.md`](docs/screening.md) — 反応データベース全体を
  スクリーニングする方法と、それが思ったより安く済む理由。
