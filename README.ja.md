...
| 問い | 必要なもの | 現在地 |
|---|---|---|
| **Q1.** どの酵素反応が環境保全寄与が大きいか | 反応クラスを横断して比較可能な指標とランキング | **指標は完了、被覆は拡大中**。製品1 kgあたりの削減量（kg-CO₂e）で順位付けできる。クラスは13個に増え、2,540/18,558件（13.7%）を照合、1,555件（8.4%）を判定確定。正直な上限は[「被覆率はどこまで伸びるか、なぜそれ以上は伸びないか」](#被覆率はどこまで伸びるかなぜそれ以上は伸びないか)を参照 |
...
1. ~~クラスを増やす——安くなったので~~ **完了、継続中。** テンプレートは
   ...
   **3件目の電荷状態の分裂**を、他の2件と同じ方法で統一した。さらに
   ...
   mCPBA工程ではなく同クラスの工程モデルを再利用した。合計：
   **2,540/18,558件を照合（13.7%）、1,555件を判定確定（8.4%）**——1クラス
