...
| 問い | 必要なもの | 現在地 |
|---|---|---|
| **Q1.** どの酵素反応が環境保全寄与が大きいか | 反応クラスを横断して比較可能な指標とランキング | **指標は完了、被覆は拡大中**。製品1 kgあたりの削減量（kg-CO₂e）で順位付けできる。クラスは3つに増え、1,370/18,558件（7.4%）を照合、739件（4.0%）を判定確定。正直な上限は[「被覆率はどこまで伸びるか、なぜそれ以上は伸びないか」](#被覆率はどこまで伸びるかなぜそれ以上は伸びないか)を参照 |
...
1. ~~クラスを増やす——安くなったので~~ **完了、継続中。** テンプレートは
   ...
   しない。同じ方式でさらに2クラスを構築した——論文の引用は一切ない。
   ...
   **判定確定は0件**——正直に報告する。工程モデルが小規模すぎて、
   ...
   試薬当量を水増しして無理に判定を出すことはしていない。合計：
   **1,370/18,558件を照合（7.4%）、739件を判定確定（4.0%）**——1クラス
