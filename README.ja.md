...
| 問い | 必要なもの | 現在地 |
|---|---|---|
| **Q1.** どの酵素反応が環境保全寄与が大きいか | 反応クラスを横断して比較可能な指標とランキング | **指標は完了、被覆は拡大中**。製品1 kgあたりの削減量（kg-CO₂e）で順位付けできる。クラスは25個に増え、8,007/18,558件（43.1%）を照合、3,876件（20.9%）を判定確定。正直な上限は[「被覆率はどこまで伸びるか、なぜそれ以上は伸びないか」](#被覆率はどこまで伸びるかなぜそれ以上は伸びないか)を参照 |
...
1. ~~クラスを増やす——安くなったので~~ **完了、継続中。** テンプレートは
   ...
   **25個目のクラス`nadh-ketoreductase.yaml`**は、上記NADPHクラスの
   ...
   本セッションの最終合計：**8,007/18,558件を照合（43.1%）、
   3,876件を判定確定（20.9%）**——セッション開始時の2,815/1,724件
   （15.2%/9.3%）から拡大した。
...
これは80〜100%がすぐに到達可能という意味ではなく、また現在の43.1%照合
／20.9%判定確定と88.6%という構造的上限とのギャップが小さい、あるいは
...
今も**照合ではなく判定確定**であり、正直に検証された20.9%は捏造された
