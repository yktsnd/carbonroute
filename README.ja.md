...
コース依存性の糖転移反応263件すべてを、実在する公表済みの化学合成手順
1つに対してスクリーニングした。かかった時間は約1秒：
...
つまり、263件の反応の閾値（85.58%〜91.43%）は、軒並みこの実プラント
水準を下回っている。言い換えると、**判定が確定した250件のうち、実際の
工場が普通に達成できる99%という回収率に耐えられるものは1件もなかった。**
だから正直な結論は「酵素が250回勝った」ではない。**「この反応クラスに
...
いるクラステンプレート1つに対して263件を採点する時間であり、純粋な
...
1つ追加するのは無料にはならない——これがどう効いてくるかは、次の
「なぜ18,558件が『解ける規模の問題』になるのか」で説明する。

このスクリーニングが本来測定したかった仕組みも、データのばらつきに
はっきり現れている。閾値は、**化学ルートが保護しなければならない基質
側の官能基の数が多いほど高くなる**——官能基が1個以下なら85.58%、33個
もある大きなオリゴ糖では91.43%だ。これは、分子構造の情報だけから
...
UDP-グルコースは上位20位にも入らない（273件）。本当に頻度が高いのは
...
必要がある。実際、同梱のUDP-グルコースのクラスでも、この検証によって
「補酵素は消費するが実は別の変換」の反応が6件見つかり、正しく除外
されている（実装の詳細は [`docs/screening.md`](docs/screening.md)）。
...
なければ、残り249件の結果は自分自身のテンプレートをなぞって報告して
