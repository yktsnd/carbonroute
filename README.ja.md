...
1. **テンプレート選定規則を定めてから、クラスを増やす**（Q1の網羅性）。
   EC-3の上位群——1.1.1（526件）、2.1.1（500件）、2.3.1（382件）——を
   追加すれば累計1,808件、Rhea全体の9.7%、EC番号付与分の23.7%に達する。
   ただし2つ目のクラスに着手して、先に潰すべき交絡が見つかった。
   テンプレートとは「誰かが選んだ論文1本」であり、アセチル化の文献には
   古典的な無水酢酸／ピリジン法と、意図的にグリーンな水系の手法の両方が
   実在する。古典的なグリコシル化テンプレートとグリーンなアセチル化
   テンプレートを組み合わせて順位を付ければ、報告されるのは酵素の差では
   なく**著者の脱溶媒への意欲の差**になる。クラス横断の比較可能性には、
   テンプレートの「年代と狙い」を揃える規則が要る——
   [`docs/screening.md`](docs/screening.md) を参照。これを明文化して
   全クラスに同じく適用してから、一次文献調査を始めるべきである。
2. **商業プロセスを位置づける**（Q3）。ヒトミルクオリゴ糖のフコシル化
