...
| 問い | 必要なもの | 現在地 |
|---|---|---|
| **Q1.** どの酵素反応が環境保全寄与が大きいか | 反応クラスを横断して比較可能な指標とランキング | **指標は完了、被覆は拡大中**。製品1 kgあたりの削減量（kg-CO₂e）で順位付けできる。クラスは10個に増え、1,971/18,558件（10.6%）を照合、1,150件（6.2%）を判定確定。正直な上限は[「被覆率はどこまで伸びるか、なぜそれ以上は伸びないか」](#被覆率はどこまで伸びるかなぜそれ以上は伸びないか)を参照 |
...
1. ~~クラスを増やす——安くなったので~~ **完了、継続中。** テンプレートは
   ...
   **判定確定は0件**——正直に報告する。工程モデルが小規模すぎて、
   ...
   （EC 2.5.1、DMAPP依存のプレニル化）は47件を照合、37件を判定確定——
   ...
   **3件目の電荷状態の分裂**を、他の2件と同じ方法で統一した。さらに
   ...
   2/3/4イソプレン単位を転移——11/13/9件を照合、9/2/3件を判定確定）を
   構築した。FPPは正直な発見である：判定確定はわずか13件中2件
   （15.4%、DMAPPの78.7%とは逆）——実際の化学の大半が鎖伸長やFPP
   ホモ二量化であり、外部求核剤への転移ではないためだ。水増しせず、
   小さいクラスは小さいクラスとして正直に報告した。合計：
   **1,971/18,558件を照合（10.6%）、1,150件を判定確定（6.2%）**——1クラス
