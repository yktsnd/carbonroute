...
| 問い | 必要なもの | 現在地 |
|---|---|---|
| **Q1.** どの酵素反応が環境保全寄与が大きいか | 反応クラスを横断して比較可能な指標とランキング | **部分的**。1クラス406件を採点済み（Rhea全体の2.2%）。クラス内の順位付けは動作するが、クラス間で比較できる指標が未定義 |
| **Q2.** 酵素反応収率・溶媒リサイクル率を考慮したときの優位性 | 収率 × 回収率の2次元の臨界曲線 | **片側のみ**。溶媒回収率の軸は完成。酵素側の収率は未実装——下記の既知バイアスを参照 |
| **Q3.** 既存のバイオものづくりの環境負荷低減効果は全体の何位か | 商業化済みプロセスとRhea反応の対応付け、および分位点 | **未着手** |

### Q2に関する既知のバイアス：現在の結果は酵素側に有利に偏っている

これは今後の課題ではなく、いま出ている数値に対する訂正である。
出荷中のテンプレートの化学側の物質量は、出典論文自身の実収率——
グリコシル化62% × 脱保護85% = **通算52.7%**——を通した値になっている。
つまり化学ルートは「1 kgの製品を得るには2倍近く仕込む必要がある」という
罰則を正しく払っている。**一方、酵素側の補酵素は純粋な化学量論、すなわち
収率100%を暗黙に仮定して課金されている**（`screen.py` の
`cofactor_kg = mol_per_fu * cofactor_coeff * cofactor_mw / 1000`）。

実際の酵素的グリコシル化が定量的に進むことは稀であり、補酵素の請求額は
収率の逆数で増える。したがって現在報告している溶媒回収率の閾値
（85.58%〜91.54%）は**上限**であり、真の値はより低い。この軸を入れる
ことは「精度の改善」ではなく、結論の向きに関わる。これがロードマップの
最優先項目である理由がここにある。

### ここから先の道筋

1. **酵素収率を第一級の変数にする**（Q2・最優先）。上の非対称性を解消し、
   出力を1次元の閾値から `(酵素収率, 溶媒回収率)` 平面上の**損益分岐
   曲線**へ拡張する。実務的に意味のある問いはこうなる：「工業的に標準的な
   90%の溶媒回収を前提としたとき、酵素法が勝ち続けるために必要な最低
   転化率は何%か？」
2. **クラス間で比較可能な指標を定義する**（Q1）。溶媒回収率の閾値は
   テンプレートの溶媒負荷に依存するためクラス間で比較できない。標準
   操業点（例：溶媒回収90%）における製品1 kgあたりの Δkg-CO₂e を区間
   として算出し、それで順位を付ける。区間は半順序しか与えないので、
   出力は偽の全順序ではなくパレート順位とする。
3. **クラスを増やす**（Q1の網羅性）。EC-3レベルの上位群——1.1.1（526件）、
   2.1.1（500件）、2.3.1（382件）——を追加すると累計1,808件、Rhea全体の
   9.7%、EC番号が付与された7,635件の23.7%に達する。各クラスに実在する
   公表済み化学手順が1本ずつ必要であり、ここは速くならない。
4. **商業プロセスを位置づける**（Q3）。ヒトミルクオリゴ糖のフコシル化
   （RHEA:14257 ほか）、アントシアニン配糖化（RHEA:20093）、β-アルブチン
   （RHEA:12560）など、既に商業生産されている酵素反応をRhea IDに対応
   付け、2.で作った順位における分位点を報告する。
...
> この3つの数値は**上限**である。化学側は出典論文の実収率52.7%を通した
> 値になっている一方、酵素側は暗黙に収率100%で課金されている。
> [「Q2に関する既知のバイアス」](#q2に関する既知のバイアス現在の結果は酵素側に有利に偏っている)を参照。
