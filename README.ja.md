...
| 問い | 必要なもの | 現在地 |
|---|---|---|
| **Q1.** どの酵素反応が環境保全寄与が大きいか | 反応クラスを横断して比較可能な指標とランキング | **指標は完了、被覆は拡大中**。製品1 kgあたりの削減量（kg-CO₂e）で順位付けできる。クラスは22個に増え、7,024/18,558件（37.8%）を照合、3,859件（20.8%）を判定確定。正直な上限は[「被覆率はどこまで伸びるか、なぜそれ以上は伸びないか」](#被覆率はどこまで伸びるかなぜそれ以上は伸びないか)を参照 |
...
1. ~~クラスを増やす——安くなったので~~ **完了、継続中。** テンプレートは
   ...
   **22個目のクラス`cmp-sialyltransferase.yaml`**（CMP-シアル酸依存の
   ...
   この単純な二反応物モデルが扱わない三反応物の形である。本セッション
   の最終合計：**7,024/18,558件を照合（37.8%）、3,859件を判定確定
   （20.8%）**——セッション開始時の2,815/1,724件（15.2%/9.3%）から
   拡大した。
...
これは80〜100%がすぐに到達可能という意味ではなく、また現在の37.8%照合
／20.8%判定確定と88.6%という構造的上限とのギャップが小さい、あるいは
...
今も**照合ではなく判定確定**であり、正直に検証された20.8%は捏造された
