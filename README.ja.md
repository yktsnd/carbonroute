...
| 問い | 必要なもの | 現在地 |
|---|---|---|
| **Q1.** どの酵素反応が環境保全寄与が大きいか | 反応クラスを横断して比較可能な指標とランキング | **指標は完了、被覆は拡大中**。製品1 kgあたりの削減量（kg-CO₂e）で順位付けできる。クラスは2つに増え、855/18,558件（4.6%）を照合。正直な上限は[「被覆率はどこまで伸びるか、なぜそれ以上は伸びないか」](#被覆率はどこまで伸びるかなぜそれ以上は伸びないか)を参照 |
...
1. ~~クラスを増やす——安くなったので~~ **完了、継続中。** テンプレートは
   ...
   しない。2つ目のクラス `sam-methyltransferase.yaml`（EC 2.1.1、SAM
   依存のO/N/S-メチル化）を同じ方式で構築した：449件を照合、351件を
   判定確定、論文の引用は一切ない。合計：**855/18,558件を照合（4.6%）、
   739件を判定確定（4.0%）**——1クラス時の406/388から拡大した。下の
   「被覆率の上限」でこの数字が伸びる範囲と伸びない理由を述べる。
