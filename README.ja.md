...
| 問い | 必要なもの | 現在地 |
|---|---|---|
| **Q1.** どの酵素反応が環境保全寄与が大きいか | 反応クラスを横断して比較可能な指標とランキング | **指標は完了、被覆は拡大中**。製品1 kgあたりの削減量（kg-CO₂e）で順位付けできる。クラスは7つに増え、1,866/18,558件（10.1%）を照合、1,073件（5.8%）を判定確定。正直な上限は[「被覆率はどこまで伸びるか、なぜそれ以上は伸びないか」](#被覆率はどこまで伸びるかなぜそれ以上は伸びないか)を参照 |
...
1. ~~クラスを増やす——安くなったので~~ **完了、継続中。** テンプレートは
   ...
   しない。同じ方式でさらに6クラスを構築した——論文の引用は一切ない。
   ...
   **1,866/18,558件を照合（10.1%）、1,073件を判定確定（5.8%）**——1クラス
   時の406/388から拡大した。下の「被覆率の上限」でこの数字が伸びる範囲と
   伸びない理由を述べる。
...
の2クラスでそれぞれ449件中98件、406件中18件を除外している）を適用する
