...
| 問い | 必要なもの | 現在地 |
|---|---|---|
| **Q1.** どの酵素反応が環境保全寄与が大きいか | 反応クラスを横断して比較可能な指標とランキング | **指標は完了、被覆は拡大中**。製品1 kgあたりの削減量（kg-CO₂e）で順位付けできる。クラスは24個に増え、7,729/18,558件（41.6%）を照合、3,876件（20.9%）を判定確定。正直な上限は[「被覆率はどこまで伸びるか、なぜそれ以上は伸びないか」](#被覆率はどこまで伸びるかなぜそれ以上は伸びないか)を参照 |
...
1. ~~クラスを増やす——安くなったので~~ **完了、継続中。** テンプレートは
   ...
   **24個目のクラス`nadph-ketoreductase.yaml`**（NADPH依存のカルボニル
   ...
   最終合計：**7,729/18,558件を照合（41.6%）、3,876件を判定確定
   （20.9%）**——セッション開始時の2,815/1,724件（15.2%/9.3%）から
   拡大した。
...
これは80〜100%がすぐに到達可能という意味ではなく、また現在の41.6%照合
